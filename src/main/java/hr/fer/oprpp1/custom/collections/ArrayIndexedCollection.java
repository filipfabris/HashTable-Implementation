...
public class ArrayIndexedCollection<T> implements List<T> {
	...
	/**
	 * <b>GetterClass </b> implements <b> ElementsGetter </b> It Gives
	 * ArrayIndexedCollection class functionality of iteration element by element
	 * 
	 * @author filip fabris
	 */
	private static class GetterClass<T> implements ElementsGetter<T> {
		...
		int currentIndex;
		/**
		 * variable to check whether there was modification during iteration on given
		 * array
		 */
		 ...
		 * Default constructor
		 * 
		 * @param savedModificationCount variable to check if there was modification on
		 *                               given collection
		 * @param collection             on which we iterate
		 * @throws NullPointerException if given collection variable is null
		 */
		GetterClass(long savedModificationCount, ArrayIndexedCollection<T> collection) {
			if (collection == null) {
		 ...
		 * @throwsConcurrentModificationException if elements in given collection was
		 *                                        modified
		 */
		@Override
	...
	public <U> void addModified(Collection<U> other, Function<? super U, ? extends T> function){
		ElementsGetter<U> iterator = other.createElementsGetter();
		
		while(iterator.hasNextElement()) {
			this.add(function.apply(iterator.getNextElement()));
		}
...
//		modificationCount++;
	...
	private void doubleSize() {
		T[] doubleElements = (T[]) new Object[size * 2];
	...
	public ElementsGetter<T> createElementsGetter() {
		return new GetterClass<T>(this.modificationCount, this);
	}
